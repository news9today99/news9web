...
export default function Home() {
  ...
  return (
    <main data-testid="home-page">
          ...
          <aside className="lg:col-span-4 space-y-6">
            <div className="bg-white border border-[#E2E8F0]" data-testid="live-tv-widget">
              ...
              <div className="p-3 border-t border-[#E2E8F0]">
                <div className="cat-tag text-[#DC2626] mb-1">{T.onAir}</div>
            ...
            <div className="bg-white border border-[#E2E8F0] p-4" data-testid="trending-widget">
              <div className="cat-tag text-[#DC2626] border-b border-[#E2E8F0] pb-2 mb-3">{T.trendingNow}</div>
      ...
      {cats.filter(c => byCat(c.slug).length > 0).map(c => (
        <CategorySection key={c.slug} title={c.name_te} slug={c.slug}
                         items={byCat(c.slug)} catLabel={c.name_te} />
      )).slice(0, 4)}
      ...
      {videos.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 mt-12">
          <div className="flex items-center justify-between border-b-2 border-[#DC2626] pb-2 mb-4">
            ...
            <Link to="/category/videos" className="cat-tag text-[#DC2626] hover:underline">{T.viewAll}</Link>
          </div>
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 mt-12" data-testid={`section-${slug}`}>
      <div className="flex items-center justify-between border-b-2 border-[#DC2626] pb-2 mb-4">
        ...
        <Link to={`/category/${slug}`} className="cat-tag text-[#DC2626] hover:underline">{T.viewAll}</Link>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
